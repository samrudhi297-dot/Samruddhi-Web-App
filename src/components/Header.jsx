...
export default function Header() {
  ...
  const [checkoutStatus, setCheckoutStatus] = useState('form') // form | success | error
  ...
  const handleCheckoutSubmit = async (e) => {
    ...
    try {
      const phone = String(checkoutForm.phone || '').trim()
      ...
      console.error(err)
      setCheckoutError(err.message || 'Could not send your request. Please try again.')
        ...
        >
          ...
          >
            ...
            <div className="d-flex justify-content-between align-items-center mb-4 pb-2" style={{ borderBottom: '1px solid #222' }}>
              <h4 style={{ color: '#FF7425', fontWeight: 'bold', margin: 0 }}>Requirement Cart</h4>
              <button 
                onClick={() => {
                  setIsCartOpen(false)
            ...
            {checkoutStatus === 'success' ? (
              <div className="text-center py-5" style={{ flex: 1 }}>
                ...
                <h5 style={{ color: '#fff', fontWeight: 'bold' }}>Quote request sent</h5>
                <p style={{ fontSize: '13px', color: '#777' }}>We’ll get back to you shortly.</p>
                <button
                  className="btn btn-sm mt-3"
                  style={{ background: '#FF7425', color: '#000', fontWeight: 'bold', border: 'none' }}
                  onClick={() => {
                    setCheckoutStatus('form')
                    setIsCartOpen(false)
                  }}
              ...
              <div className="text-center py-5" style={{ flex: 1 }}>
                <h5 style={{ color: '#ff6b6b', fontWeight: 'bold' }}>Couldn’t send request</h5>
                <p style={{ fontSize: '13px', color: '#777' }}>{checkoutError}</p>
                <button
                  className="btn btn-sm mt-3"
                  style={{ background: '#FF7425', color: '#000', fontWeight: 'bold', border: 'none' }}
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  {cartItems.map((item) => (
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: '12px', color: '#FF7425' }}>{item.price}</div>
                ...
                <div style={{ borderTop: '1px solid #222', paddingTop: '20px' }}>
                  <h5 style={{ color: '#FF7425', fontSize: '16px', fontWeight: 'bold', marginBottom: '15px' }}>Request Quotation</h5>
                  <form onSubmit={handleCheckoutSubmit}>
                    ...
                    <button 
                      ...
                      style={{ background: '#FF7425', border: 'none', color: '#000', fontWeight: 'bold', padding: '8px' }}
                    >
